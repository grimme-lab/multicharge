...
subroutine mchrg_ssytrf(amat, ipiv, uplo, info)
   ...
   integer(i8), intent(out), optional :: info
   character(len=1) :: ula
   integer(i8) :: stat, n, lda, lwork, stat_alloc, stat_dealloc
...
subroutine mchrg_ssytrs(amat, bmat, ipiv, uplo, info)
   ...
   integer(i8), intent(out), optional :: info
   character(len=1) :: ula
   integer(i8) :: stat, n, nrhs, lda, ldb
   if (present(uplo)) then
      ula = uplo
   else
      ula = 'u'
...
subroutine mchrg_ssytri(amat, ipiv, uplo, info)
   ...
   integer(i8), intent(out), optional :: info
   character(len=1) :: ula
   integer(i8) :: stat, n, lda, stat_alloc, stat_dealloc
   real(sp), allocatable :: work(:)
   if (present(uplo)) then
      ula = uplo
   else
