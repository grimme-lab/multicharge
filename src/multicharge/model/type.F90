...
module multicharge_model_type
   ...
   abstract interface 
      ...
      subroutine get_amat_3d(self, mol, cache, wsc, alpha, amat)
         import :: mchrg_model_type, structure_type, mchrg_cache, &
...
subroutine solve(self, mol, cn, qloc, dcndr, dcndL, dqlocdr, dqlocdL, &
   ...
   call self%update(mol, grad.or.cpq, cache)
   !if(allocated(cache%cmat)) then 
   !   cache%cmat = 0.0_wp
   !   do iat = 1, mol%nat
   !      cache%cmat(iat, iat) = 1.0_wp
   !   end do 
   !
   !end if
   !if(allocated(cache%dcdr)) then 
   !   cache%dcdr = 0.0_wp
   !   cache%dcdL = 0.0_wp
   !end if
   ...
   call self%get_vrhs(mol, cache, cn, qloc, xvec, dcndr, dcndL, &
      & dqlocdr, dqlocdL, dxdr, dxdL)

   if (any(mol%periodic)) then
      call self%get_amat_3d(mol, cache, wsc, alpha, amat)
   else
      call self%get_amat_0d(mol, cache, cn, qloc, amat)
   end if
